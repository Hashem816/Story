...
async def admin_prod_price(message: types.Message, state: FSMContext, is_operator: bool):
    """استقبال سعر المنتج وإنهاء الإضافة"""
    if not is_operator:
        return
    ...
    try:
        ...
        await db_manager.log_admin_action(
            ...
            target_type="PRODUCT",
            details=f"إنشاء منتج: {data['name']} - {price}$"
        )
...
async def admin_prod_view(callback: types.CallbackQuery, is_operator: bool):
    ...
    type_text = {"MANUAL": "يدوي", "AUTOMATIC": "تلقائي", "DISABLED": "معطل"}.get(product['type'], product['type'])
    ...
    builder = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="✏️ تعديل", callback_data=f"admin_prod_edit_{product_id}")],
...
async def admin_prod_edit_menu(callback: types.CallbackQuery, state: FSMContext, is_operator: bool):
    ...
    builder = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="📝 تعديل الاسم", callback_data=f"admin_prod_edit_name_{product_id}")],
        [types.InlineKeyboardButton(text="📄 تعديل الوصف", callback_data=f"admin_prod_edit_desc_{product_id}")],
        [types.InlineKeyboardButton(text="💰 تعديل السعر", callback_data=f"admin_prod_edit_price_{product_id}")],
        [types.InlineKeyboardButton(text="⚙️ تعديل النوع", callback_data=f"admin_prod_edit_type_{product_id}")],
        [types.InlineKeyboardButton(text="🔙 عودة", callback_data=f"admin_prod_view_{product_id}")]
    ])
